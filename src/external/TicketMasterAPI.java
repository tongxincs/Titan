...
public class TicketMasterAPI implements ExternalAPI {
	 ...
	 */
	// search method is the key method in this project, search the surrounding events
	@Override
	public List<Item> search(double lat, double lon, String term) {
		// initial url to know where we will search
		...
		// initial query with following infomation 
		...
		try {
			...
			// when we running, it will get connection
			...
			String inputLine;
			// we want manipulate the string, and save space, we will change it to JSON format
			...
			// we transfer the response to a JSON format
	...
	// we will make sure the url do not have some special characters, we need encode the term
	...
	// input is latitude and longtutide
	private void queryAPI(double lat, double lon) {
		List<Item> itemList = search(lat, lon, null);
		try {
			for (Item item : itemList) {
				JSONObject jsonObject = item.toJSONObject();
				// print each item we found as JSON format
